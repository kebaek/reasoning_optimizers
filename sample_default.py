...
for x in tqdm(eval_questions):
  ...
  preds.append(default_sampling(tokens, model, tokenizer, temperature=1.0))
acc, answer_types_all = evaluate(preds, eval_answers, True)

answer_types_all, preds = np.array(answer_types_all), np.array(preds)
np.save('default_answer_types_all.npy', answer_types_all)
np.save('default_preds.npy', preds) 
